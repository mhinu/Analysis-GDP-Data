...
#filter the data of the financial year 2015-16 of karnataka data total gsdp

statewise_gsdp %>% 
  filter(state=="Karnataka") %>% 
  filter(year=="2015-16") %>% 
  group_by(state) %>% 
  summarise(total_gsdp = sum(gsdp, na.rm=T)) -> karnataka_gsdp
karnataka_gsdp$total_gsdp -> only_karnataka
print(only_karnataka)

statewise_gsdp %>% 
  summarise(total_gsdp=sum(gsdp, na.rm=T)) -> total_gsdp
print(total_gsdp)

((only_karnataka/total_gsdp)*100)-> percentage
paste( "The Karnataka state alone has a contribution of",percentage,"% in overall GDP ")
